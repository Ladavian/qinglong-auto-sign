...
# sign_daily_task/main.py -> sign_daily_task -> smzdm -> repository root
...
try:
    # Normal path when invoked through ql_smzdm.py.
    from ..api.api import SmzdmAPI
    from .service import SmzdmService
except ImportError:
    # Keep compatibility with direct execution of this source file.
    from api.api import SmzdmAPI
    from service import SmzdmService
...
def setup_logger():
    ...
    file_handler = logging.FileHandler(
        os.path.join(project_root, 'smzdm_task.log'), encoding='utf-8', mode='a'
    )
...
class SmzdmTaskManager:
    ...
    def __init__(self):
        ...
        """
        # The bundled version defaults to its own configuration file.  The
        # environment variable makes the location configurable in QingLong.
        root_path = Path(project_root)
        config_path = Path(
            os.environ.get("SMZDM_CONFIG_PATH")
            or os.environ.get("smzdm_config_path")
            or root_path / "config" / "smzdm_token.json"
        )
